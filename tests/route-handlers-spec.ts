...
describe("API Endpoints", () => {
  ...
  describe("POST /v1/create/appointment/:barbershop/:barber/", () => {
    it("should create an appointment", (done) => {
      const services = [
        { price: 20, duration: 30, service: "Child’s Haircut (12 and under)" },
      ];
      ...
      const appointmentTime = moment("2019-09-30 14:00")
        .add(getRandomInt(1, 100000), "day")
        .format("YYYY-MM-DD HH:mm");
      const expectedTime = moment(appointmentTime, `YYYY-MM-DD HH:mm`).format(
        friendlyFormat
      );
      ...
      sandbox.stub(client.messages, "create").callsFake((createdMessage) => {
        ...
        const expectedMessage = !messageCount
          ? expectedMessageOne
          : expectedMessageTwo;
        messageCount++;
  ...
  describe("GET /v1/get/schedule/:barbershop/:barber", () => {
    it("should retrieve barber schedule", (done) => {
      ...
      request
        .get(
          `/v1/get/schedule/barberSharp/${barber}?totalDuration=60&fromDate=${fromDate}`
        )
        .expect(200)
        .end(function (err, res) {

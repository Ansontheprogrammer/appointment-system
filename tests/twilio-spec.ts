  ...
  describe("getAvailableTimes", () => {
    it("should not be able to book an appointment during a lunch time", () => {
      const availableTimes = getAvailableTimes(
        ...
        "2019-12-12",
        "2019-12-13",
        exampleBarber as any,
        shopAvailability
      );
      const timesThatShouldBeUnavailable = availableTimes.filter(
        (availableTime) =>
          availableTime.time === "10:00" || availableTime.time === "10:15"
      );
    ...
    it("should be able to book an appointment if it is not lunch time or off day", () => {
      ...
      const timesThatShouldBeUnavailable = availableTimes.filter(
        (availableTime) =>
          availableTime.time === "10:30" || availableTime.time === "11:00"
      );
      const expectedTimeAvailability = [
    ...
    it("should not be able to book an appointment during an off day", () => {
      ...
      const areAllTimesUnavailable = availableTimes.every(
        (availableTime) => availableTime.available === false
      );
